...
# Helper functions
# =========================================================
def sd_path(filename):
    """
    Return filename under SD_MOUNT unless it is already absolute.
    """
    if filename.startswith("/"):
        return filename

    return SD_MOUNT + "/" + filename

def transfer_filename(transfer_id, transfer_type):
    if transfer_type == TRANSFER_TYPE_ANIMATION:
        return sd_path("anim_{:08x}.dlt".format(transfer_id & 0xFFFFFFFF))

    return sd_path("transfer_{:08x}.bin".format(transfer_id & 0xFFFFFFFF))
...
def register_builtin_animation_files():
    filename = sd_path(DEFAULT_ANIM_FILE)

    if file_exists(filename):
        try:
            anim_id = file_crc32(filename)
            ANIM_FILES[anim_id] = filename
            print(
        ...
        print("Default animation file not found:", filename)


def get_free_flash_bytes():
    """
    Returns approximate free bytes on the SD card, or None if statvfs
    is unavailable.
    """
    try:
...
def resolve_animation_file(anim_id):
    transferred = sd_path("anim_{:08x}.dlt".format(anim_id & 0xFFFFFFFF))
